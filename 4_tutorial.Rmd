...
**Note:** The predict function in this code will return "out-of-bag" predictions for every municipality because we did not give it a new prediction data set, like we did in the section [EA-level Predictions] (see`?predict.randomForest` for more info). The "out-of-bag"" predictions for a municipality are generated from only the regression trees from the model that did not include that municipality in their randomly sampled training data. This provides a cross-validation (a.k.a. out-of-bag, or out-of-sample) assessment.
...
If you want to view the EA-level population estimates on a map (e.g. Fig. \@ref(fig:in-out)), you will need to join the model results (from section [EA-level Predictions] above) to polygon features for use with GIS software (e.g. Esri polygon shapefile).  
...
**Note:** We have demonstrated parallel processing using the EA-level data in `master_predict`. It is important to note that EA-level predictions do not take too long to compute using the simpler approach (see section [EA-level Predictions]). We recommend parallel processing only for larger tasks like generating gridded predictions from `mastergrid_predict`. We did not use this data in our example because it would potentially introduce complications that would distract from the demonstration of parallel processing, but the workflow that we demonstrated should be easily adaptable to accommodate any larger data sets. 
