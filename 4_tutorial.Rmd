...
If you want to view the EA-level population estimates on a map, you will need to join the model results (from section [EA-level Predictions] above) to polygon features for use with GIS software (e.g. Esri polygon shapefile).  
...
Then, we need to load a vector GIS data set containing the polygons that we would like to join our model results to.

```{r, eval=F}
sf_polygons <- st_read('BR_Setores_2019.shp')
```

For this example, we are using Brazilian census enumeration area polygons which are openly available online as an Esri polygon shapefile [@ibge2020population]. Note that the `st_read` function can read most GIS file formats (e.g. Esri shapefile, geopackage, geojson).  See `?sf::st_read` for more information.
...
```{r, eval=F}
sf_polygons_proj <- st_transform(sf_polygons, as.character(crs(raster_covariate))) #we first convert it to the same crs
...
# Suggested Citation {-}
Leasure DR, Darin E, Tatem AJ. 2020. Top-down disaggregation of municipality population totals to estimate populations in smaller enumeration areas: An R tutorial. WorldPop, University of Southampton. doi:10.5258/SOTON/WP00697.
