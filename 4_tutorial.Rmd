...
This model will be implemented in the R programming language [@r2020r]. Our first step is to setup the R environment so that it contains the R packages and data that we will need. The `randomForest` package [@law2018] implements the random forest algorithm in R. If you have not already installed it, then you will need to install it from the R console.
...
1. **master_train.csv** Training data for the model that contains population totals and covariate values for 5,562 municipalities in Brazil, and

2. **master_predict.csv** Prediction data for the model that contains covariate values for 444,045 enumeration areas where population estimates are needed.  
...
2. Brazil's [2007 census totals](https://biblioteca.ibge.gov.br/index.php/biblioteca-catalogo?view=detalhes&id=293420) for municipalities,  
...
```{r hist, echo=F, eval=T, out.align="center", out.width="75%", fig.cap="Histogram of log-transformed population density."}
...
We have now disaggregated the population totals from 5,562 municipalities in Brazil into population estimates for 444,045 enumeration areas (Fig. \@ref(fig:in-out)).
...
2.  Build a test model using aggregated population counts at a courser resolution (e.g. state-level)  and evaluating its predictions at the municipality level.
